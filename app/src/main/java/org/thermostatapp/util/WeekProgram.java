...
public class WeekProgram {
    ...
    public Map<String, ArrayList<SwitchHS>> data = new HashMap<String, ArrayList<SwitchHS>>();
    ...
    public void setDefault() {
        ...
        for (int i = 0; i < this.valid_days.length; i++) {
            ...
            this.data.put(day, new ArrayList<SwitchHS>());
            this.data.get(day).add(new SwitchHS("night", false, "00:00"));
            this.data.get(day).add(new SwitchHS("night", false, "00:00"));
            this.data.get(day).add(new SwitchHS("night", false, "00:00"));
            this.data.get(day).add(new SwitchHS("night", false, "00:00"));
            this.data.get(day).add(new SwitchHS("night", false, "00:00"));
            this.data.get(day).add(new SwitchHS("day", false, "00:00"));
            this.data.get(day).add(new SwitchHS("day", false, "00:00"));
            this.data.get(day).add(new SwitchHS("day", false, "00:00"));
            this.data.get(day).add(new SwitchHS("day", false, "00:00"));
            this.data.get(day).add(new SwitchHS("day", false, "00:00"));
    ...
    public String toXML() throws NullPointerException {
        ...
        for (int i = 0; i < this.valid_days.length; i++) {
            ...
            // Add the switches.
            ArrayList<SwitchHS> switches = this.data.get(day);
            if (switches != null) {
                for (SwitchHS sw : switches) {
    ...
    public boolean duplicates(ArrayList<SwitchHS> switches) {
...
*/
    ...
    //Setting switches. Switches list should always exactly consist out of 10 elements.
     ...
     //* @param switches_list
     //* @param nr_switches
    public void setSwitches(String day, ArrayList<SwitchHS> switches_list,
                            int nr_switches) {
        // Validate input???
